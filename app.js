...
async function extractDraftRows(file) {
  ...
  for (let pageNo = 1; pageNo <= pdf.numPages; pageNo++) {
    ...
    const playerGroups = groupByBaseline(
      items.filter(item => item.x >= playerX + 5 && item.x < roundX - 10),
      3.5
    );

    const playerCandidates = playerGroups
      .map(g => ({
        y: g.y,
        text: normalizeText(g.items.sort((a, b) => a.x - b.x).map(x => x.str).join(" "))
      }))
      .filter(g => g.text)
      .filter(g => !isNonPlayerText(g.text));
    ...
    for (const group of groups) {
      ...
      // In this PDF layout the actual player name is on the line immediately
      // ABOVE the Round/Pick/Manager metadata. The club + position line is
      // immediately BELOW the player name. A pure "nearest text line" lookup
      // is therefore unsafe: depending on font metrics, the club line can be
      // slightly closer to the metadata row.
      //
      // Prefer the closest candidate at or above the metadata baseline. This
      // reliably selects "B.Fernandes" rather than "MUN MID", "Haaland"
      // rather than "MCI FWD", etc.
      const above = playerCandidates
        .filter(candidate => candidate.y <= group.y + 2.5)
        .map(candidate => ({
          ...candidate,
      ...
      // Fallback for PDFs whose text-layer baselines differ: use the nearest
      // valid candidate, but still exclude club/position lines.
      const nearby = above.length ? above : playerCandidates
        .map(candidate => ({
          ...candidate,
